	    ...
	    if (resource_class === "asset"){
	    	...
	    	// With ID 
	    		...
	    		var result = $.grep(lists, function (o){
		    		console.log("id = " + o.id);
		    		return o.id === parseInt(resource_id);
		    	});
		    	object = result[0];
	    	}
	    	...
	    	lists = listOfmessage;
			// undefined???
			console.log("lists = " + listOfmessage + "(" + lists.length +")");
	    	// With ID
		    var result = $.grep(lists, function (o){
		    	console.log("message with id = " + o.id);
		    	return o.id === parseInt(resource_id);
		    }); 
		    //console.log("lists.length = " + lists.length + " result.length = " + result.length);
			if (result.length < 1)
				console.log("Error! Message not found!");
			else object = result[0];
	    }
	...
	function passAttributes(event){
		...
		$(".add-new-asset").on('click', addNewAsset);
	...
	function createIndividual(knowledgepointid, editmode){
		...
		var header = $('<h1>'+KnowledgePoint.name+'</h1>');
		...
		$.each(assets, function (i, item){
			...
			var oneitem = $('<li>');
			var oneasset = $('<a href="#individual-2nd-popup">' + path + '</a>');
			// On click -> build 2nd popup
			oneasset.on('click', function (event){
				clear2nd();
				//var asset_location = "../Asset/COMP_2611/Lecture_1/L1.pdf";
				var asset_location = path;
				var start = item.startpoint;
				var end = item.endpoint;

				var title = $('<h1>'+asset_location.substring(asset_location.lastIndexOf("/") + 1, asset_location.length)+"{from: " +start + ", to: " + end + "}"+'</h1>');
				var type = asset_location.substring(asset_location.lastIndexOf(".") + 1, asset_location.length);
				...
				if (type === "pdf"){
					...
					content = $('<audio controls><source data="' + asset_location +"#t=" + start + ','+ end + '" type="audio/' + type + '"></audio>');
				}
		...
		if (tests.length > 0){
			...
			var test_button = $('<input type="submit" value="Add new tests">');
		...
		$.each(messages, function (i, item){
			...
			/*if (item.ans.length > 0){
				var answer = item.ans[0].content;
				console.log("item.ans[0] = " + JSON.stringify(item.ans[0]) + "item.ans[0].content = " + item.ans[0].content);
				oneasset = $('<li><a id="q-'+ item.id +'">' + question + '</a><a id="a-'+ item.id + '">'+ answer + '</a></li>');
			} else {*/
				oneasset = $('<li><a id="q-'+ item.id +'">' + question + '</a></li>');
			//}
	...
	function createVideopopup(videoid, editmode){
		...
		var id = asset.id;
		//var path = asset.path;
		var path = "../Asset/COMP_2611/Lecture_1/COMP2611.mp4";
		...
		var source = $('<source src="' + path + '" type="video/' + format + '">');
		...
		$("#tag-tool-video-popup").popup('open');
	...
	function createAudiopopup(audioid, editmode){
		...
		var source = $('<source src="' + path + '" type="audio/' + format + '">');
		...
		// $("#tag-tool-video-popup").addClass('popup-visible').popup('open');
		$("#tag-tool-audio-popup").popup('open');
	...
	function displayMessageBox (data){
		...
		$.when($.each(listOfmessage, function (i, v) {
			...
			var messagehtml = $('<li id="message-'+v.id+'" draggable="true"><span id="message-q-'+v.id+'">'+v.question+'</span></li>');
	...
	function createAssetList(list){
		...
		$.each(list, function (i){
			...
			var name = list[i].slice(list[i].indexOf('">'), list[i].length - 3);
			var type = list[i].slice(-3, list[i].length);
			var oneasset = new asset(-1, path, type, -1, -1, null, null);
	...
	function displayAssetList(list){
		...
		$.when($.each(list, function (i, v) {
			...
			var assethtml = $('<li id="asset-'+temp_id+'" draggable="true" class="asset" src="'+v.path+'">'+path+'</li>');
			assethtml.on('dragstart', drag);
			assethtml.on('click', passAttributes);
			//messagehtml.draggable({ opacity: 0.7, helper: "clone" });
	...
	function addNewAssetIntoHTML(newasset){
		...
		var temp_id = newasset.id
		var assethtml = $('<li id="asset-'+temp_id+'" draggable="true" class="asset" src="'+newasset.path+'">'+path+'</li>');
		assethtml.on('dragstart', drag);
		assethtml.on('click', passAttributes);
		//messagehtml.draggable({ opacity: 0.7, helper: "clone" });
		assethtml.appendTo($(".all-assetslist"));
...
$(document).ready(function(){
	...
	canvas.on('mouse:down', function(o){
		...
		if (e != null && e.get('type') === 'group'){
			...
			clearIndividual();
			createIndividual(selected_KnowlodgePoint_id, dragdropMode);
			$("#individual-popup").popup('open');
		}
	});
