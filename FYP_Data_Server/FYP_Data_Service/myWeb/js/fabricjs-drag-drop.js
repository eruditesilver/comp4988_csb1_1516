	...
	function drop(event) {
	    ...
	    // push into KP
	    var x = (event.originalEvent.clientX - ($('#canvas').offset().left + $('#canvas').scrollLeft())) - 2 + $(document).scrollLeft();
	    var y = (event.originalEvent.clientY - ($('#canvas').offset().top + $('#canvas').scrollTop())) - 2 + $(document).scrollTop();
	    console.log("drag y = " + y + " y - doc = " + y - $(document).scrollTop());
	...
	// fill pdf popup
	function createPDFpopup(pdfid, isOpen){
		if (!isOpen) return;
		...
		$("#tag-tool-pdf-popup").modal('show');

		popupPDFloaded(path);
	}
	
	function createVideopopup(videoid, isOpen){
		if (!isOpen) return;
		...
		var source = $('<source src="' + assetsURL + name + '" type="video/' + format + '">');
		...
		$("#tag-tool-video-popup").modal('show');
	...
	function createAudiopopup(audioid, isOpen){
		if (!isOpen) return;
		...
		var source = $('<source src="' + assetsURL + name + '" type="audio/' + format + '">');
		...
		// $("#tag-tool-video-popup").addClass('popup-visible').popup('open');
		$("#tag-tool-audio-popup").modal('show');
	...
	function clearTagtoolpopup(){
		...
		$(".tag-end").val("");
		// remove red
        if ($( ".tag-start" ).hasClass("has-error")){
			$( ".tag-start" ).removeClass("has-error");
		}
        if ($( ".tag-end" ).hasClass("has-error")){
			$( ".tag-end" ).removeClass("has-error");
		}
		$(".asset-name").val("");
	}
	...
	function droponCanvas(object_class, object, x, y){
		...
		canvas.forEachObject(function(o){ 
			if (o.get('type') === 'group') {
	            ...
	            var radius = parseInt(o.item(0).get('radius'));
	            console.log("point = ("+ groupPos.x + ", " + groupPos.y + ", " + (groupPos.x+radius) + ", " + (groupPos.y+radius) + ")");
	...
	function addNewAsset(event){
		...
		var start = $(".modal.in .tag-start").val();
		var end = $(".modal.in  .tag-end").val();
		...
		if (parseInt(start) > parseInt(end)){
			...
			alert("Please input valid range!");
			return;
		}
		//console.log("end = " + end);
